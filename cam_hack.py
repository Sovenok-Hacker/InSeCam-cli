...
data = requests.get(f'http://www.insecam.org/en/bycountry/{country.upper()}/', headers={'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:97.0) Gecko/20100101 Firefox/97.0'}).content.decode()
soup = BS(data, 'lxml')
log = open('cameras.txt', 'w')
pages_count = int(str(soup.find('ul', class_='pagination')).split('\n')[2][34:][:3])
for i in range(0, pages_count):
    ...
    for camera in soup.find_all(class_='thumbnail-item__img img-responsive'):
        if camera.get('src').endswith('.mjpg'):
